...
long time_stop(struct timeval &tv1, struct timeval &tv2, struct timeval &dtv)
{
...
int main(int argc, char **argv)
{
	...
	while (true)
	{
		...
		for (int i = pinfo.zfrom; i < pinfo.ztill; i++)
		{
			for (int j = pinfo.yfrom; j < pinfo.ytill; j++)
			{
				...
				{
					pinfo.refreshBorders(i, j, k, partF, prevIt);
	...
	long msec = time_stop(tv1, tv2);

	pinfo.findMaxValue(msec);
	pinfo.findMaxValue(max);

	if (rank == 0)
	{
		std::cout << "Elapsed in " << msec << " msec" << std::endl
			<< "With max differ: " << max << " in "
